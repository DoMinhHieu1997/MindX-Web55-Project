...
const Recipes = () => {
  ...
  const getListRecipes = (type) => {
    ...
    setIsLoading(true);
    fetch(
      `${COMMON.DOMAIN}posts?s=${number ? 8 * page : 8}&t=1&p=${
        number ? 1 : page + 1
      }`
    )
      .then((res) => res.json())
      .then((resJson) => {
        if (resJson.data.length % 8 !== 0 || !resJson.data.length)
    ...
    <>
      ...
      <div className="container py-5">
        <div className="d-flex align-items-center justify-content-center">
          ...
          <h3 className="text-center text-uppercase fw-normal mx-3 mb-0">
            Công thức cho bạn
          </h3>
          <RestaurantIcon
            sx={{ fontSize: 40 }}
            style={{ color: "#6c757d", transform: "rotateY(180deg)" }}
          />
        </div>
        <div
          className="bg-secondary mx-auto mt-3"
          style={{ height: "3px", width: "5rem" }}
        ></div>
        ...
        {isLoading ? (
          <div className="post-loading row mt-3">
            <Skeleton />
          </div>
        ) : null}
        {displayLoadMore && (
